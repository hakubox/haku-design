...
import { type Component, type ComponentGroup, type FormDimensionItem, ComponentCategory } from '@haku-design/core';
...
export const service = {
  ...
  countScore() {
    if (editorState.appConfig.questionnaireConfig.hasScore) {
      ...
      let _totalScore = 0;
      /** 维度配置 */
      const _dimensions: FormDimensionItem[] = editorState.appConfig.questionnaireConfig.dimensionConfig?.isOpen
        ? editorState.appConfig.questionnaireConfig.dimensionConfig?.dimensionList
        : [];

      _components
        .filter((item) => !item.isGroup && [ComponentCategory.normal, ComponentCategory.complex].includes(item.type))
        .forEach((item) => {
          // 普通题
          // if (item.attrs.score) {
