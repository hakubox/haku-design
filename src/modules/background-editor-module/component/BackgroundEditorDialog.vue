<template>
  ...
  >
    ...
    <div class="background-dialog-content">
      <!-- 线性渐变 -->
      <!-- 径向渐变 -->
      <div class="gradient-config" v-if="state.currentBackgroundType !== 'image' && state.currentBackgroundType !== 'color'">
        <GradientSlider
          :cursor-list="state.gradientList"
          v-model:current-cursor-index="state.currentGradientItemIndex"
        />
      </div>
      <!-- 纯色 -->
      <TypeColorPicker v-if="state.currentBackgroundType !== 'image'" />
    </div>
  </HakuDialog>
</template>
...
import type { AppBackgroundType, GradientItem } from '../index.d';
...
const state = reactive({
  /** 当前背景类型 */
  currentBackgroundType: 'color' as AppBackgroundType,
  /** 当前背景类型索引 */
  ...
  /** 当前渐变的子项列表 */
  gradientList: [] as GradientItem[],
  /** 当前渐变项索引 */
  currentGradientItemIndex: -1,
});

/** 修改背景类型 */
const setBackgroundType = (name: AppBackgroundType) => {
  ...
  if (_typeIndex >= 0) {
    ...
    switch (state.currentBackgroundType) {
