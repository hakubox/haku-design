...
import type { ErrorInfo, FormInfoItem, TempStorage, TimerInfo, TimingInfo } from './src/@types';
...
export const service = {
  ...
  async validateForm(formPageIndex?: number): Promise<{ isComplete: boolean; errorComponents: any[] }> {
          ...
          if (component.attrs.validate) {
            try {
              await validateService
                .transformValidate(_val, component.attrs.validate)
                .then((d) => {
                  console.log('成功');
                })
                .catch(({ errors }) => {
    ...
    if (_errorComponents.length === 0) {
      ...
      setTimeout(() => {
        const _el = document.querySelector(`.form-canvas.preview [component-id="${_errorComponents[0].component.id}"]`);
