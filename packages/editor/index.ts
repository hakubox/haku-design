...
import { createModelId, isBlank, isNotBlank, recursive, timeout } from '@haku-design/common';
import { addQuestionary, saveQuestionary } from "@haku-design/core/src/api/questionnaire";
...
export const service = {
  ...
  async loadAppBody(questionaryId: string, body: ExportAppBody, isPreview: boolean = false) {
    try {
      if (!isPreview) service.changeSelectedFormComponent([]);
      const hide = message.toast('数据加载中...', 'loading');
      ...
      if (!isPreview) message.toast('加载完成', 'success');
    } catch(err) {
      message.toast('加载数据出错', 'error');
      console.error(err);
    }
  },
  /** 切换当前选择的控件 */
  ...
  saveQuestionnaire() {
    ...
    const hide = message.toast(_canUpgrade ? '问卷更新中...' : '问卷保存中...', 'loading', 0);
    saveQuestionary(_exportData, _canUpgrade ? 'UPDATE' : 'UPGRADE').then(d => {
      ...
      setTimeout(() => {
        message.toast(_canUpgrade ? '问卷更新成功' : '问卷保存成功', 'success');
      }, 300);
    }).catch(([err]) => {
      message.toast(err.desc || '保存问卷出错', 'error');
    }).finally(() => {
      hide();
    });
  },
